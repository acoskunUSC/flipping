...
public class ConflictGenerator {
    ...
    private static int indexOfRobots = 0;
	...
	static private AgentMission generateMission(ExtensibleProblem problem,
        ...
        // check the biggest possible x and y position of the vertices
        ...
        for (int i=0; i<possibleEndpoints.size(); i++) {
            if (possibleEndpoints.toArray(new Point[0])[i].x >= biggestX) {
                biggestX = possibleEndpoints.toArray(new Point[0])[i].x;
            } else if (possibleEndpoints.toArray(new Point[0])[i].x <= smallestX) {
                smallestX = possibleEndpoints.toArray(new Point[0])[i].x;
            }
            if (possibleEndpoints.toArray(new Point[0])[i].y >= biggestY) {
                biggestY = possibleEndpoints.toArray(new Point[0])[i].y;
            } else if (possibleEndpoints.toArray(new Point[0])[i].y <= smallestY) {
                smallestY = possibleEndpoints.toArray(new Point[0])[i].y;
            }
        }

        int firstQuarterX = smallestX + (biggestX-smallestX)/4;
        int lastQuarterX = smallestX + 3*((biggestX-smallestX)/4);
        int firstQuarterY = smallestY + (biggestY-smallestY)/4;
        int lastQuarterY = smallestY + 3*((biggestY-smallestY)/4);
        int middleY = smallestY + ((biggestY-smallestY)/2);
        int middleX = smallestX + ((biggestX-smallestX)/2);

        boolean isStartOnTheSide = true;
        boolean isTargetOnTheSide = true;
        ...
        do {
            ...
            if (GenerateEAInstance.separateRobotsExperiment1) {

                if (indexOfRobots % 5 == 1 || indexOfRobots % 5 == 2) {
                    isStartOnTheSide = (start.x > 50 && start.x < 700 && start.y > 50 && start.y < 300);
                } else if (indexOfRobots % 5 == 0) {
                    isStartOnTheSide = (start.x > 200 && start.x < 750 && start.y > 2250 && start.y < (2400-50));
                } else if (indexOfRobots % 5 == 3 || indexOfRobots % 5 == 4) {
                    isStartOnTheSide = (start.x > 2500 && start.x < 3000 && start.y > 1650 && start.y < 1950);
                }

            }

            if (GenerateEAInstance.separateRobotsExperiment2and3) {

                if (indexOfRobots % 2 == 1) {
                    isStartOnTheSide = (start.x < 350 && start.x > 50 && start.y < middleY);
                } else {
                    isStartOnTheSide = (start.x > biggestX - 350 && start.x < biggestX - 50 && start.y < middleY);
                }

            }
            
        } while (collidesWithOtherStart || collidesWithOtherGoal || connectivityLost || !isStartOnTheSide);
        ...
        do {
            ...
            if (GenerateEAInstance.separateRobotsExperiment1) {
                ...
                if (indexOfRobots % 5 == 1 || indexOfRobots % 5 == 2) {
                    isTargetOnTheSide = (target.x > 1950  && target.x < 2800 && target.y > 1950 && target.y < (2400-50));
                } else if (indexOfRobots % 5 == 0) {
                    isTargetOnTheSide = (target.x > 1550  && target.x < 2350 && target.y > 650 && target.y < 1050);
                } else if (indexOfRobots % 5 == 3 || indexOfRobots % 5 == 4) {
                    isTargetOnTheSide = (target.x > 50  && target.x < 650 && target.y > 1650 && target.y < 1950);
                }
            ...
            if (GenerateEAInstance.separateRobotsExperiment2and3) {
                ...
                if (indexOfRobots % 2 == 1) {
                    isTargetOnTheSide = (target.x > biggestX - 350 && target.x < biggestX - 50 && target.y > lastQuarterY && target.y < biggestY - 50);
                } else {
                    isTargetOnTheSide = (target.x > 50  && target.x < 350 && target.y > lastQuarterY && target.y < biggestY - 50);
                }

            }
            
        } while (start == target || collidesWithOtherGoal || collidesWithOtherStart || connectivityLost || !isTargetOnTheSide);
